-- BEAST MODE — Migração 043: motor de ciclismo, espelhando o motor de corrida
-- (036/037). Reaproveita os mesmos campos de avaliação (distancia_alvo_km,
-- superficie_corrida, desnivel_corrida, data_prova) em vez de duplicar schema —
-- "superfície de estrada/trail/desnível" descreve o evento-alvo independentemente
-- da modalidade, não é exclusivo de corrida apesar do nome da coluna. Categoria
-- (Curta/Média/Longa/Ultra) é sempre CALCULADA a partir da distância, nunca
-- guardada como escolha própria — mesmo princípio da 036. (aplicada 2026-08-18)

create or replace function bm_classificar_ciclismo(p_km numeric) returns text
language sql immutable set search_path = public as $$
  select case
    when p_km is null then null
    when p_km <= 20 then 'Iniciação'
    when p_km <= 50 then 'Curta'
    when p_km <= 100 then 'Média distância'
    when p_km <= 160 then 'Longa'
    else 'Ultra / Gran Fondo'
  end
$$;

create or replace function desc_formato_ciclismo(p_formato text, p_taper boolean) returns text
language plpgsql set search_path = public as $$
declare v_base text;
begin
  v_base := case p_formato
    when 'intervalada' then 'Séries de potência: 5-6x 3 min a esforço forte (zona 5), recuperação de 3 min em rotação leve entre séries.'
    when 'fartlek' then 'Ritmo variado sem plano fixo: alterna troços fortes e fáceis conforme o terreno, sem parar.'
    when 'longa' then 'Saída longa a ritmo fácil e conversacional (zona 2) — o objetivo é tempo em cima da bike, aumenta a duração 5-10% por semana.'
    when 'subida' then 'Repetições em subida: 5-6x 4-6 min a esforço forte a subir sentado, descer em rotação leve como recuperação — força específica para desnível.'
...
begin
  ...
  -- deliberada; o ciclismo continua a entrar como complemento mais abaixo).
  ...
  if v_so_corrida then
    ...
    if v_aval.modalidades ? 'ciclismo' then
      ...
      v_desc_ciclismo := desc_formato_ciclismo(v_formato_ciclismo, v_em_taper);
      v_dias := v_dias || jsonb_build_object(
        'nome', 'Ciclismo', 'nome_exibicao', 'Ciclismo' || coalesce(' (' || v_categoria_ciclismo || ')', ''),
        'subtitulo', 'Complemento cardio',
        'tipo', 'ciclismo', 'formato', v_formato_ciclismo, 'descricao', v_desc_ciclismo,
        'local', v_local_ciclismo, 'duracao_min', v_aval.minutos_por_sessao,
