-- BEAST MODE — Migração 011: motor gera reavaliação (12 semanas), dia de estímulo (15 dias)
-- rotativo, e dia de corrida quando a modalidade está selecionada. (aplicada 2026-08-01)
...
begin
  ...
  if v_aval.modalidades ? 'corrida' then
    ...
    v_dias := v_dias || jsonb_build_object(
      ...
      'formato', v_formato_corrida, 'local', v_local_corrida,
      'duracao_min', v_aval.minutos_por_sessao
    );
  end if;

  select count(*) into v_planos_anteriores from planos where cliente_id = p_cliente_id;
  select * into v_wod from wods_referencia order by id offset (v_planos_anteriores % 4) limit 1;
  v_dias := v_dias || jsonb_build_object(
