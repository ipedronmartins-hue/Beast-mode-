-- BEAST MODE — Migração 039: Session Builder. Cada sessão ganha fases reais
-- (aquecimento → trabalho principal → acessório → core → arrefecimento) em vez de
-- ser uma lista plana. Regra determinística usando dados que já existem — os primeiros
-- 2 padrões compostos (squat/hinge/push/pull) do dia são "principal", os restantes
-- compostos e todo o isolamento são "acessório", core fica à parte. Sem taxonomia nova.
-- (aplicada 2026-08-11)
